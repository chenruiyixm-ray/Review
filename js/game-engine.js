...
(function (global) {
  ...
  function enterLevel(levelNum) {
    ...
    // ===== 构建配对数据 =====
    // 问题：原始数据中可能存在
    //   (a) 相同词义出现多次（如"怨恨"x2）→ 右侧词义加 ①②③
    //   (b) 相同句子出现多次但词义不同（如"强国请服…"x2 → "降服"/"诸侯国"）
    //       → 左侧句子加 ①②③，让用户能区分
    //
    // 第一步：统计每个词义 & 每个句子的出现次数
    var meaningCount = {};
    var sentenceCount = {};
    for (var s = 0; s < lv.sentences.length; s++) {
      var m = lv.sentences[s][1];
      var t = lv.sentences[s][0];
      meaningCount[m] = (meaningCount[m] || 0) + 1;
      sentenceCount[t] = (sentenceCount[t] || 0) + 1;
    }

    // 第二步：为每个句子和词义分配序号
    var meaningIndex = {};   // 词义 → 当前已分配序号
    var sentenceIndex = {};  // 句子 → 当前已分配序号
    ...
    for (var idx = 0; idx < lv.sentences.length; idx++) {
      ...
      var rawSentence = item[0];
      var leftId = 'L' + idx;
      var rightId = 'R' + idx;

      // 词义序号
      var mSeq = meaningIndex[rawMeaning] || 0;
      meaningIndex[rawMeaning] = mSeq + 1;

      // 句子序号
      var sSeq = sentenceIndex[rawSentence] || 0;
      sentenceIndex[rawSentence] = sSeq + 1;

      // 右侧词义显示：重复时加 ①②③
      var displayMeaning = rawMeaning;
      if (meaningCount[rawMeaning] > 1) {
        displayMeaning = rawMeaning + ' ' + toSuperscript(mSeq + 1);
      }

      // 左侧句子显示：重复时加 ①②③
      // 注意：句子高亮由 highlightChar 处理，编号追加在句子末尾
      var displaySentence = rawSentence;
      if (sentenceCount[rawSentence] > 1) {
        displaySentence = rawSentence + ' ' + toSuperscript(sSeq + 1);
      }

      leftItems.push({
        id: leftId,
        text: displaySentence,      // 可能带 ① 后缀
        rawText: rawSentence,      // 原文（用于高亮）
        answerId: rightId,
        matched: false
      });
  ...
  function renderGameScreen() {
    ...
    for (var a = 0; a < leftItems.length; a++) {
      ...
      // 用 rawText 做高亮（不带编号后缀），编号单独用 <sup> 追加
      var sentenceHtml = highlightChar(li.rawText || li.text, lv.character);
      // 如果 text 比 rawText 多了编号后缀，提取并追加
      var rawT = li.rawText || li.text;
      var fullT = li.text;
      if (fullT.length > rawT.length) {
        var suffix = fullT.substring(rawT.length); // e.g. " ①"
        sentenceHtml += suffix.replace(/ (①|②|③|④|⑤|⑥|⑦|⑧|⑨|⑩)$/, ' <sup class="sentence-suffix">$1</sup>');
      }
      html += '<div class="' + cls + '" data-id="' + li.id + '" data-side="left"><div class="sentence">' + sentenceHtml + '</div></div>';
    }
